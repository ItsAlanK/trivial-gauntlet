...
As the page loads the player is immediately given a concise rundown of the game's basic rules and is presented the choice of difficulty which then launches the player straight into the game with no extra page loads.
...
### Wireframes ###
The following are the wireframes used to plan the layout for the game's main pages. The mobile and desktop versions will remain largely the same with some elements stacking vertically rather than horizontally on smaller devices. This will be achieved using css flexbox and css grids.
- Mobile Title Page
